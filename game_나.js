...
function perform(){
    ...
    birdList.forEach((bird, idx, obj)=>{
       ...
       // collistion 함수를 하나 만들어서 고양이 위치와 bird의 위치를 비교해서
       // x값과 y값이 곂치면(고양이x - 새x < 0) = 음수 > alert('충돌')
       collision(cat, bird);
    ...
    if(jump_flag == flase){
        if(cat.y < 200){        // 원래 위치로 복귀하면 중지 (더내려가지 않게 하기 위함)
            cat.y += 4
        }
    }
...
document.addEventListener('keydown', (e)=>{
    // 스페이스가 눌렸는지 검사한다.
    // 고양이의 위치가 기본위치 일때만 jump_flag 만 true 시킨다
    if(e.code === 'space'){
        if(isGameRunning == true){   // 기본 위치일대만 점프
            if(cat.y === 200){
                jump_flag = true
            }else{
                startGame()
...
function collision(고양이, 새){
    //var x차이 = 새.x - 고양이.x;    // 새는 x가 감소하는 중
    //var y차이 = 새.y - 고양이.y;    // 
...
function startGame(){
    // 재시작할 때 초기화 해줘야하는 부분을 처리(처음 설정으로)
    jump_flag = false
    jump_timer = 0
    birdList = []
